      ...
      <div className="auth-content" style={{ flexDirection: 'column' }}>
        <div className="auth-card fade-in glass-panel">
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <h2 style={{ fontSize: '1.4rem', fontWeight: 600 }}>Create your account</h2>
            <p style={{ color: 'var(--text-muted)' }}>Join ConnectSphere today.</p>
          </div>
          {error && <div className="alert alert-error">{error}</div>}
          <form className="auth-form" onSubmit={handleSubmit}>
            <div className="form-group">
              <input id="reg-name" name="name" type="text" className="form-input"
                placeholder="Full Name (e.g. Ahmed Sufyan)" value={form.name} onChange={handleChange} required />
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              <input id="reg-password" name="password" type="password" className="form-input"
                placeholder="Password (Min 6 characters)" value={form.password} onChange={handleChange} required />
            </div>
            <div className="form-group">
              <input id="reg-bio" name="bio" type="text" className="form-input"
                placeholder="A short bio about yourself (optional)" value={form.bio} onChange={handleChange} />
            </div>
          ...
          <div style={{ textAlign: 'center' }}>
            <Link to="/login" className="btn btn-outline" style={{ padding: '10px 24px' }}>Already have an account? Sign In</Link>
          </div>
        </div>
      </div>
    </div>
